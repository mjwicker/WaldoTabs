...
async function updateCacheCount() {
  const resp = await browser.runtime.sendMessage({ action: "getCachedState" });
...
document.getElementById("viewCache").addEventListener("click", async () => {
  const resp = await browser.runtime.sendMessage({ action: "getCachedState" });
  console.log("[TabOptimizer] Cache:", resp.cache);
  setStatus(`${resp.cache.length} tab(s) cached. See console for details.`);
});
