...
See `~/Documents/Waldo/Wiki/projects/waldo-tabs/roadmap.md` for full roadmap, queue state,
...
- **Observability:** `lib/observability.js` (browser logger + `waldoTabsEvents` storage ring);
  Node tests use `logging_utils.js` → `waldotabs_data/logs/`. See `OBSERVABILITY.md`.
...
4. Update `Wiki/task-queue.md` task status if applicable (🔵 → ✅)
...
2. Update `Wiki/projects/waldo-tabs/roadmap.md` if queue state changed
3. Update `Wiki/task-queue.md` task status (⚪ → 🔵 when starting, 🔵 → ✅ when done)
